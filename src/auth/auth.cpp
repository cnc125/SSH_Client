...
namespace {
    constexpr uint8_t SSH_MSG_SERVICE_REQUEST = 5;
    constexpr uint8_t SSH_MSG_SERVICE_ACCEPT = 6;
    constexpr uint8_t SSH_MSG_USERAUTH_REQUEST = 50;
    constexpr uint8_t SSH_MSG_USERAUTH_FAILURE = 51;
    constexpr uint8_t SSH_MSG_USERAUTH_SUCCESS = 52;
}

std::vector<uint8_t> Auth::create_service_request() const {
    std::vector<uint8_t> payload;
    payload.push_back(SSH_MSG_SERVICE_REQUEST);
...
void Auth::validate_service_accept(const std::vector<uint8_t>& payload) const {
    ...
    if (payload[0] != SSH_MSG_SERVICE_ACCEPT) {
...
std::vector<uint8_t> Auth::create_none_auth_request(const std::string& username) const {
    std::vector<uint8_t> payload;
    payload.push_back(SSH_MSG_USERAUTH_REQUEST);
...
void Auth::validate_auth_success(const std::vector<uint8_t>& payload) const {
    if (payload.size() != 1 || payload[0] != SSH_MSG_USERAUTH_SUCCESS) {
